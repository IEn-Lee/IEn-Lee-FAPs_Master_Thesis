...
static LowViscosityRampPlanner low_viscosity_planner;
static bool low_viscosity_planner_valid = false;
...
// --- Low viscosity ramp parameters ---
// These parameters are used only for low-viscosity extrusion curve planning.
// The velocity limit still comes from Customized Parameters.
static constexpr float LOW_VISCOSITY_MAX_ACCELERATION_MM_S2 = 0.08f;
static constexpr float LOW_VISCOSITY_MAX_DECELERATION_MM_S2 = 0.08f;
static constexpr float LOW_VISCOSITY_START_VELOCITY_MM_S    = 0.0f;
static constexpr float LOW_VISCOSITY_STOP_VELOCITY_MM_S     = 0.0f;
static constexpr float LOW_VISCOSITY_FIRST_VELOCITY_MM_S    = 0.03f;
static constexpr float LOW_VISCOSITY_FIRST_ACCEL_MM_S2      = 0.02f;
...
// --------------------------------------
static bool build_low_viscosity_ramp_planner_from_current_settings()
{
    low_viscosity_planner_valid = false;

    if (!recorded_quantity) {
        return false;
    }

    LowViscosityRampPlanner::Params p;

    p.quantity_uL = atof(recorded_quantity);
    ...
    p.max_acceleration_mm_s2 = LOW_VISCOSITY_MAX_ACCELERATION_MM_S2;
    p.max_deceleration_mm_s2 = LOW_VISCOSITY_MAX_DECELERATION_MM_S2;

    p.start_velocity_mm_s = LOW_VISCOSITY_START_VELOCITY_MM_S;
    p.stop_velocity_mm_s  = LOW_VISCOSITY_STOP_VELOCITY_MM_S;

    p.first_velocity_mm_s = LOW_VISCOSITY_FIRST_VELOCITY_MM_S;
    p.first_acceleration_mm_s2 = LOW_VISCOSITY_FIRST_ACCEL_MM_S2;

    if (p.stroke_per_ml_mm <= 0.0f) {
        p.stroke_per_ml_mm = 13.0f;
    }

    if (p.max_velocity_mm_s <= 0.0f) {
        p.max_velocity_mm_s = 1.0f;
    }

    if (p.max_acceleration_mm_s2 <= 0.0f) {
        p.max_acceleration_mm_s2 = 5.0f;
    }

    if (p.max_deceleration_mm_s2 <= 0.0f) {
        p.max_deceleration_mm_s2 = 5.0f;
    }

    if (!low_viscosity_planner.build(p)) {
        return false;
    }

    low_viscosity_planner_valid = true;

    recorded_duration_sec_exact = low_viscosity_planner.totalDuration();
    ...
    realtime_curve_volume_max_uL = p.quantity_uL;
    if (realtime_curve_volume_max_uL < 1.0f) {
        realtime_curve_volume_max_uL = 1.0f;
    }

    realtime_curve_current_max_A = 5.0f;
...
static void start_btn_event_cb(lv_event_t* e)
{
    ...
    if (!is_running) {
        ...
        if (low_viscosity_mode_active) {
            ...
            if (!build_low_viscosity_ramp_planner_from_current_settings()) {
                MotionModeManager::forceIdle("Failed to build low viscosity planner");
                low_viscosity_mode_active = false;
                show_missing_param_dialog("Failed to build low viscosity planner");
                return;
            }
...
void show_start_confirm_dialog(lv_obj_t* start_btn)
{
    ...
    lv_obj_add_event_cb(btn_yes, [](lv_event_t* e){
        ...
        if (low_viscosity_mode_active)
        {
            if (!low_viscosity_planner_valid) {
                is_running = false;
                MotionModeManager::forceIdle("Low viscosity planner invalid");
                low_viscosity_mode_active = false;
                show_missing_param_dialog("Low viscosity planner invalid");
                return;
            }
        ...
        if (low_viscosity_mode_active &&
            low_viscosity_planner_valid &&
            RealtimeCurveRenderer::isInitialized()) {

            LowViscosityRampPlanner::Sample s0 =
                low_viscosity_planner.evaluate(0.0f);
...
static bool build_realtime_spi_streamer_from_current_settings()
{
    ...
    track_cfg.max_acceleration = 2.0f;
    track_cfg.start_velocity = 0.0f;
    track_cfg.stop_velocity = 0.0f;
...
void loop()
{
    ...
    if (is_running && RealtimeCurveRenderer::isInitialized()) {
        if (millis() - last_curve_update_ms >= 50) {
            ...
            if (low_viscosity_mode_active && low_viscosity_planner_valid) {
                LowViscosityRampPlanner::Sample sample =
                    low_viscosity_planner.evaluate(t_s);
            ...
            else if (realtime_motion_planner_valid) {
                PlungerMotionPlanner::Sample sample =
                    realtime_motion_planner.evaluate(t_s);

                float current_A = sample.currentEst;

                if (realtime_streamer_valid) {
                    current_A =
                        realtime_streamer.getStatus().measured_current_mA / 1000.0f;
                }
