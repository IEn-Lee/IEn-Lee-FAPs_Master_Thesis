...
FlowConstraintModel::Result FlowConstraintModel::compute(const Params& p)
{
    ...
    if (p.max_linear_speed_mm_s > 0.0f) {
        v_user_speed = p.max_linear_speed_mm_s / 1000.0f;
        Q_user_speed = out.A * v_user_speed;
    }
    ...
    float Q_hardware_speed_safe =
        safe_positive(p.beta) * min(out.Q_rpm, out.Q_step);
    ...
    if (Q_user_speed > 0.0f) {
        out.Q_speed_safe = min(Q_hardware_speed_safe, Q_user_speed);
    } else {
